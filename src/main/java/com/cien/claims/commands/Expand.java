...
public class Expand extends CienCommandBase {
	...
	public void onCommand(ICommandSender sender, EntityPlayerMP player, String[] args) {
		if (args.length < 1) {
			player.addChatMessage(new ChatComponentText(Util.fixColors(Util.getErrorPrefix()+"Uso: /expand <Tamanho>")));
		} else {
			int tamanho;
			try {
			...
			if (claimToExpand == null) {
				player.addChatMessage(new ChatComponentText(Util.fixColors(Util.getErrorPrefix()+"Fique sobre um claim para expandir")));
				return;
			}
			if (!claimToExpand.getOwner().equals(player.getCommandSenderName())) {
				player.addChatMessage(new ChatComponentText(Util.fixColors(Util.getErrorPrefix()+"Você tem que ser dono do terreno para expandir")));
				return;
			}
			...
			if (CienClaims.CLAIMS.getBlocksOf(player.getCommandSenderName()) - (claim.getSize()-claimToExpand.getSize()) < 0) {
				player.addChatMessage(new ChatComponentText(Util.fixColors(Util.getErrorPrefix()+"Blocos insuficientes. (Necessário "+(claim.getSize()-claimToExpand.getSize())+")")));
				return;
			}
			...
			player.addChatMessage(new ChatComponentText(Util.fixColors(Util.getPrefix()+"Claim Expandido Com Sucesso!")));
		}
	}
