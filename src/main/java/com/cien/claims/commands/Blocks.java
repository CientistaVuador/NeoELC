...
public class Blocks extends CienCommandBase {
	...
	public void onCommand(ICommandSender sender, EntityPlayerMP player, String[] args) {
		if (args.length == 0) {
			player.addChatMessage(Util.fixColors(Util.getPrefix()+"Blocos de Claim: "+CienClaims.CLAIMS.getBlocksOf(player.getCommandSenderName())));
		} else {
			if (CienPermissions.PERMISSIONS.hasPermission(player.getCommandSenderName(), "admin.blocks")) {
				if (args.length < 3) {
					player.addChatMessage(Util.fixColors(Util.getErrorPrefix()+"Uso: /blocks <al/ad/re> <Player> <Blocos>"));
				} else {
					...
					if (!Properties.hasProperties(args[1])) {
						player.addChatMessage(Util.fixColors(Util.getErrorPrefix()+"Player Inválido."));
						return;
					}
					switch (op) {
					case "al":
