...
public class TrustList extends CienCommandBase {
	...
	public void onCommand(ICommandSender sender, EntityPlayerMP player, String[] args) {
		...
		if (set.size() == 0) {
			player.addChatMessage(Util.fixColors(Util.getErrorPrefix()+"Não há ninguém com trust no seu claim."));
			return;
		}
		player.addChatMessage(Util.fixColors(Util.getPrefix()+"Lista de Trust em "+f.getId()+":"));
		for (Entry<String, Integer> e:set) {
			String level = "Entrada";
			if (e.getValue() == com.cien.claims.Claim.TRUST) {
				level = "Trust";
			}
			player.addChatMessage(Util.fixColors(" §6"+e.getKey()+", Permissão de "+level));
		}
	}
