...
public class TpClaim extends CienCommandBase {
	...
	public void onCommand(ICommandSender sender, EntityPlayerMP player, String[] args) {
		if (args.length == 0) {
			player.addChatMessage(new ChatComponentText(Util.fixColors(Util.getErrorPrefix()+"Uso: /tpclaim <ID>")));
		} else {
			int id;
			try {
			...
			if (world == null) {
				player.addChatMessage(new ChatComponentText(Util.fixColors(Util.getErrorPrefix()+"Mundo inválido ou não carregado.")));
				return;
			}
			int y = world.getHeightValue(x, z);
			Util.teleportPlayer(player, world, x, y, z, player.rotationPitch, player.rotationYaw);
			player.addChatMessage(new ChatComponentText(Util.fixColors(Util.getPrefix()+"Teleportado Com Sucesso!")));
		}
	}
