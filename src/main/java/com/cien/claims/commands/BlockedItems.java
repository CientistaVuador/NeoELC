...
public class BlockedItems extends CienCommandBase {
	...
	public void onCommand(ICommandSender sender, EntityPlayerMP player, String[] args) {
		...
		if (blocked.length == 0) {
			player.addChatComponentMessage(Util.fixColors(Util.getErrorPrefix()+"Não há nenhum item bloqueado."));
		} else {
			player.addChatComponentMessage(Util.fixColors(Util.getPrefix()+"Itens Bloqueados: "));
			for (String b:blocked) {
				String[] split = b.split(":");
				int id = Integer.parseInt(split[0]);
				int meta = Integer.parseInt(split[1]);
				ItemStack stack = new ItemStack(Item.getItemById(id), 1, meta);
				player.addChatComponentMessage(Util.fixColors(" §6"+stack.getDisplayName()));
			}
		}
	}
