...
public class TransferirClaim extends CienCommandBase {
	...
	public void onCommand(ICommandSender sender, EntityPlayerMP player, String[] args) {
		...
		if (args.length != 1) {
			player.addChatMessage(Util.fixColors(Util.getErrorPrefix()+"Uso: /transferirclaim <Player>"));
			return;
		}
		if (f == null) {
		...
		if (transferir == null) {
			player.addChatMessage(Util.fixColors(Util.getErrorPrefix()+"Player Inválido ou Offline"));
			return;
		}
		f.setOwner(transferir.getCommandSenderName());
		f.removeAllFlagsWith(player.getCommandSenderName());
		player.addChatMessage(Util.fixColors(Util.getPrefix()+"Claim transferido."));
	}
