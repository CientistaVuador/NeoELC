...
public class Staff extends CienCommandBase {
	...
	public void onCommand(ICommandSender sender, EntityPlayerMP player, String[] args) {
		if (CienPermissions.PERMISSIONS.hasPermission(sender.getCommandSenderName(), "chat.staff")) {
			if (args.length <= 0) {
				player.addChatMessage(new ChatComponentText(Util.fixColors(Util.getErrorPrefix()+"Uso: /s <Mensagem>")));
				return;
			}
			...
			if (muteTime > 0) {
				...
				String message = Util.fixColors(CienChat.CHAT.buildStaffChatMessageFor(player.getCommandSenderName(), msg));
