...
public class Vip extends CienCommandBase {
	...
	public void onCommand(ICommandSender sender, EntityPlayerMP player, String[] args) {
		if (CienPermissions.PERMISSIONS.hasPermission(sender.getCommandSenderName(), "chat.vip")) {
			if (args.length <= 0) {
				player.addChatMessage(Util.fixColors(Util.getErrorPrefix()+"Uso: /vip <Mensagem>"));
				return;
			}
			...
			if (muteTime > 0) {
				...
				String message = CienChat.CHAT.buildVipChatMessageFor(player.getCommandSenderName(), msg);
